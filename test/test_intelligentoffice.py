...
class TestIntelligentOffice(unittest.TestCase):
    ...
    def test_check_quadrant_occupancy_not_valid_pin(self):
        ...
        self.assertRaises(IntelligentOfficeError,
                          self.intelligent_office.check_quadrant_occupancy,
                          not_valid_infrared_pin)

    @patch.object(IntelligentOffice, "change_servo_angle")
    @patch.object(SDL_DS3231, "read_datetime")
    def test_manage_blinds_based_on_time_open(self, read_datetime_mock: Mock,
    ...
    def test_manage_blinds_based_on_time_saturday_sunday(self, read_datetime_mock: Mock,
        ...
        read_datetime_mock.side_effect = [datetime(2024, 10, 5, 20, 0, 0),
                                          datetime(2024, 10, 6, 20, 0, 0)]
    ...
    def test_manage_light_level_turn_on(self, led_sensor_mock: Mock,
        ...
        with patch("mock.adafruit_veml7700.VEML7700.lux", PropertyMock()) as mock_lux:
            ...
            led_sensor_mock.assert_called_once_with(self.intelligent_office.LED_PIN,
                                                    True)
    ...
    @patch.object(GPIO, "input")
    def test_manage_light_level_turn_off_light_when_all_workers_off(self,
                                                                    infrared_sensor_mock: Mock, ):
        with patch("mock.adafruit_veml7700.VEML7700.lux", PropertyMock()) as mock_lux:
            mock_lux.return_value = 499.0
            infrared_sensor_mock.return_value = False
    ...
    @patch.object(GPIO, "input")
    @patch.object(GPIO, "output")
    def test_monitor_light_turn_on_buzzer_if_gas_detected(self, buzzer_mock: Mock,
                                                          gas_smoke_detector: Mock):
    ...
    @patch.object(GPIO, "input")
    @patch.object(GPIO, "output")
    def test_monitor_light_turn_off_buzzer_if_gas_not_detected(self, buzzer_mock: Mock,
                                                               gas_smoke_detector: Mock):
        gas_smoke_detector.return_value = False
        self.intelligent_office.monitor_air_quality()

        gas_smoke_detector.assert_called_once_with(self.intelligent_office.GAS_PIN)
        buzzer_mock.assert_called_with(self.intelligent_office.BUZZER_PIN, False)
        self.assertFalse(self.intelligent_office.buzzer_on)
